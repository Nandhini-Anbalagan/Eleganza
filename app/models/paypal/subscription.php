...
$lang = $_POST['lang'];
$agent_id = $_POST['id'];
$agent_id_ofc = IDObfuscator::encode($_POST['id']);
...
if(verifyCC()){
	...
	$response = payment(($_POST['subscription']+$amount), "Subscription Fee + Ad Campaign ($area)");
...
<body>
					...
					<table>
						<tr>
							<td colspan="2" align="right">
								<?php echo $lang == "EN"?'Invoice #':'# Facture'; ?>: <a href='<?php echo WEBSITE_URL ?>/receipt/<?php echo $response['transactionID'] ?>'><?php echo $response['transactionID'] ?></a><br>
					...
					<table>
						...
						<tr>
							<td colspan="2">
								...
								<?php echo  $email . "<br>" ?>
								<?php echo "<b>". ($lang == "EN"?'Account No.: ':'No. de compte: '). "</b>". $agent_id_ofc . "<br>"?>
								<?php echo "<b>". ($lang == "EN"?'Sector: ':'Secteur: ') . "</b>". $area ?>
							</td>
						</tr>
					</table>
				</td>
	...
	<?php
		...
		if(isset($_POST['reccuent']))
			$db->resetRecurringDate($agent_id, $user_id);
